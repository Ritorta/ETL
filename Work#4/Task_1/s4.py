...
with open('C:\\Users\\Esdesu\\Desktop\\JreJre\\ETL\\password.txt', 'r') as pas:
    password = pas.read()
...
if 1==1:
	...
	.option("usePlainNumberFormat","true")\
        ...
        .option("header", "true")\
        .format("excel")\
        .load("C:\\Users\\Esdesu\\Desktop\\JreJre\\ETL\\HomeWork\\ETL\\Work#4\\Task_2\\s4.xlsx").where(col("title") == "news")\
        .withColumn("dt", current_timestamp())\
        .write.format("jdbc").option("url","jdbc:mysql://localhost:3306/spark?user=root&password=" + password)\
        .option("driver", "com.mysql.cj.jdbc.Driver").option("dbtable", "tasketl4a")\
        .mode("append").save()
...
df1 = spark.read.format("com.crealytics.spark.excel")\
	...
	.option("usePlainNumberFormat","true")\
        ...
        .option("header", "true")\
        .format("excel")\
        .load("C:\\Users\\Esdesu\\Desktop\\JreJre\\ETL\\Working\\W4\\s4_2.xlsx").limit(1000)\
        .withColumn("проценты", sum1(col("Платеж по процентам")).over(w))\
        .withColumn("долг", sum1(col("Платеж по основному долгу")).over(w))
df1.write.format("jdbc").option("url","jdbc:mysql://localhost:3306/spark?user=root&password=" + password)\

...
def WetherETL():
    ...
    yandex_temperature = ti.xcom_pull(task_ids=["yandex_wether"], key="wether")[0]["temperature"]
    yandex_datetime = ti.xcom_pull(task_ids=["yandex_wether"], key="wether")[0]["datetime"]
    open_temperature = ti.xcom_pull(task_ids=["open_wether"], key="open_wether")[0]["temperature"]
    open_datetime = ti.xcom_pull(task_ids=["open_wether"], key="open_wether"][0]["datetime"]

    # Формирование данных в виде списка
    data = [
        ["Yandex:", f"{yandex_temperature} degrees", yandex_datetime],
        ["Open Weather:", f"{open_temperature} degrees", open_datetime]
    ]

    # Форматирование таблицы с помощью tabulate
    table = tabulate(data, headers=["Source", "Temperature", "Datetime"], tablefmt="grid")

    # Формирование текста сообщения
    text = f"*Weather in Tyumen*\n{table}"
